...
public class ModemDAO {
    ...
    public void saveValuesToDB() throws IOException {
        ...
        jdbcTemplate.update("INSERT INTO params VALUES (DEFAULT, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
                , map.get("ebNo"), map.get("unitAlarm"), map.get("txAlarm"), map.get("temperature"), map.get("rxAlarm"), map.get("rsl")
                , map.get("ber"), map.get("txPowerLevelIncrease"), map.get("oduAlarm"), map.get("ebNoRemote"), modem.getId(), map.get("timestampWotz"));
